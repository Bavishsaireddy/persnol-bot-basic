...
_resume_keywords = [kw.lower() for kw in _routing.get("resume_keywords", [])]
...
def intake_node(state: AgentState) -> AgentState:
    ...
    Priority (all rules from config.yaml):
      1. Message contains a resume keyword   → "resume"
      2. Message contains a profile keyword  → "profile"
      3. Message contains a tool keyword     → "retrieve"
      4. Short message, no keywords          → "direct"
      5. Fallback                            → default_route from config
    """
    ...
    has_resume  = any(kw in msg for kw in _resume_keywords)
    ...
    logger.debug(
        "intake_node | session=%s | len=%d | resume=%s | profile=%s | keyword=%s → route=%s",
        state.get("session_id"), len(msg), has_resume, has_profile, has_keyword, route,
    )
